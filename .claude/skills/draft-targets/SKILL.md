---
name: draft-targets
description: Search for draft prospects or IFA signings matching natural language criteria (position, age, ceiling, tools, personality, college/HS, nation).
...
To prevent context bleedover between searches, you MUST delegate this entire task to a
fresh Agent (subagent_type: "general-purpose", model: "sonnet"). Pass all instructions
below to the agent. Do NOT do the work inline in the current conversation.
...
Show up to 15 rows. Then you MUST print the following cost line on its own line — this is required:

`~ Model: claude-sonnet-4-6 | ~8–15K in / ~3–5K out | est. 7–12¢`
